...
2. Подготовьте USB-flash drive объемом не менее 2 Гб.
...
5. Запишите ISO-образ на USB-flash drive.
6. В параметрах BIOS компьютера выберите загрузку с USB-flash drive и начните загрузку системы с приготовленного носителя. На экране монитора должен появиться установщик Ideco UTM, как проиллюстрировано ниже.
...
![](../.gitbook/assets/install-menu.png)

### Этап 2. Начало установки

Выберите диск для установки, введя номер диска, который хотите использовать.

![](../.gitbook/assets/install-disk-change.png)
...
![](../.gitbook/assets/install-disk-changed.png)

Далее необходимо выбрать часовой пояс. Выберите временную зону, в которой вы находитесь и введите **y**.

![](../.gitbook/assets/install-city.png)

В следующем диалоговом окне необходимо настроить дату и время. Если настройки верны, то введите **y**.

![](../.gitbook/assets/install-time.png)

Если настройки не актуальны, то введите **n** и укажите верные данные, после чего нажмите **Enter**.

![](../.gitbook/assets/install-time-change.png)

Далее система произведет необходимые для установки тесты дисковой подсистемы. Создание таблицы разделов и их форматирование будет осуществлено в автоматическом режиме. После завершения создания разделов начнется копирование системных файлов на диск. Копирование проходит в автоматическом режиме. После его завершения мастер установки автоматически настроит основные параметры системы в соответствии с конфигурацией вашего компьютера.
...
![](../.gitbook/assets/install-welcome.png)
...
![](../.gitbook/assets/install-password-create.png)

**Если пароль не будет соответствовать требованиям политики безопасности, то откроется окно с выводом ошибки.** Необходимо ввести новый пароль, учитывая требования к созданию паролей и нажать **Enter**.

![](../.gitbook/assets/install-error-password.png)

После того как аккаунт будет создан, перейдите в локальное меню и введите данные от только что созданной учетной записи администратора.

![](../.gitbook/assets/installed-sign-in.png)

### Этап 3. Настройка сетевого адаптера

Теперь система предложит вам настроить локальный сетевой интерфейс. Локальным считается интерфейс, к которому будет подключена локальная сеть вашего предприятия.  Введите номер сетевого адаптера из списка и нажмите **Enter**. Второй свободный адаптер будет использован для подключения к сети вашего интернет-провайдера. 
...
![](../.gitbook/assets/install-change-disk.png)
...
![](../.gitbook/assets/install-ip-created.png)
