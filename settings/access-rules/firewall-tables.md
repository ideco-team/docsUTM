...
1\. В нужной таблице нажмите кнопку **Добавить**.

2\. Укажите необходимые параметры, действия правила и нажмите кнопку **Добавить**.

Правило будет добавлено в конец списка. Если необходимо, измените его приоритет кнопками ![](/.gitbook/assets/icon-arrow-up.png) и ![](/.gitbook/assets/icon-arrow-down.png).
...
* **Источник**:
  ...
  * **Инвертировать HIP-профиль** - позволяет использовать в правиле все объекты, кроме выбранных в строке **HIP-профили**.
...
* **Назначение**:
  ...
  * **Оператор `И`** - позволяет выбрать второе условие для трафика. При выборе второго условия правило срабатывает только в случае, если оба условия соответствуют **Назначению**.
...
* **Дополнительно**:
  ...
  * **Логировать** - включите логирование для записи срабатывания правила в [**Журнал трафика**](/settings/reports/traffic-log.md).
...
* **Дополнительно**:
  * **Включить правило** - включите правило. По умолчанию правило выключено.
  * **Логировать** - включите логировнаие для записи срабатывания правила в [**Журнал трафика**](/settings/reports/traffic-log.md).
  * **Время действия** - время действия правила. Указываются временные промежутки (например, **рабочее время**), которые определяются в [**Объектах**](aliases.md).
  * **Комментарий** - произвольный текст, поясняющий цель действия правила. Значение не должно быть длиннее 255 символов.
