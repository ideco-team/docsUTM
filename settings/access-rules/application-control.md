...
{% hint style="success" %}
Название службы раздела _Контроль приложений_: `ideco-app-backend` и `ideco-app-control@Leth<номер локального интерфейса>`.\
...
Чтобы подготовиться к изменениям, обновите версию до 17.5 (если у вас 17.4 и ниже), создайте профили в разделе **Профили безопасности -> [Профили контроля приложений](/settings/security-profiles/application-control/README.md)** и выберите их в правилах **[Файрвола](/settings/access-rules/firewall.md)**: они не будут влиять на работу существующих правил Файрвола и Контроля приложений, но начнут работать после обновления на 18 версию.
{% endhint %}

{% hint style="info" %}
Правила разделов [Предотвращения вторжений](ips/README.md), [Контроля приложений](application-control.md) и [Ограничение скорости](shaper.md) не обрабатывают трафик между локальными сетями и сетями филиалов.
...
Сессии, установленные, пока **Контроль приложений** был отключен, не будут проверены после включения модуля. Если необходимо, чтобы **Контроль приложений** их проверил, нужно вручную разорвать эти сессии и установить заново.
{% endhint %}
...
{% hint style="warning" %}
Не рекомендуем создавать в **Контроле приложений** наиболее приоритетное запрещающее правило со значениями **Любой** в полях **Применяется для** и **Протокол**. Это приведет к тому, что веб-интерфейс NGFW и доступ к серверу по SSH станут недоступны.
{% endhint %}
