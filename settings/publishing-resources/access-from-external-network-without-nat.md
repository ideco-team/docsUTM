...
Для примера разберем настройку фаервола для доступа без NAT к IP-адресу: **10.0.0.1** \(в общем случае это также может быть сеть или диапазон IP-адресов\).

1\. Выключите параметр **Автоматический SNAT локальных сетей** в **Правила доступа -&gt; Фаервол**.

2\. В фаерволе в таблице SNAT создайте правило с действием **Не производить SNAT** для данного IP-адреса назначения.
...
**Итоговые правила фаервола выглядят так:**
...
Для примера разберем настройку фаервола разрешающую хосту 10.0.0.1 доступ к локальной сети.
