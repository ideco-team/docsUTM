...
2\. Переведите переключатель раздела **BGP** в положение **Включен**. Модуль BGP запустится, если будет настроен хотя бы один BGP-сосед.
...
Проверьте, попадает ли этот адрес в список подсетей в адресе назначения, для которых применяется SNAT (указаны в предустановленном правиле в разделе **Правила трафика -> Файрвол -> SNAT**). Если IP-адрес Ideco NGFW VPP, используемый для BGP-соседства, не входит ни в одну из указанных подсетей в поле **Адрес назначения**, отредактируйте правило: добавьте этот IP-адрес в поле **Адрес назначения**. Убедитесь, что включена опция **Инвертировать назначение**.
{% endhint %}
...
* **Назначение** - IP-адрес интерфейса Ideco VPP, на котором настраивается текущее правило DNAT, используемый для BGP-соседства;
