...
В NGFW предусмотрено два режима работы WCCP - L2 и GRE. Для выбора режима раскройте блок **Настройки**. Для работы WCCP в GRE-режиме необходимо также создать на Ideco NGFW [GRE-интерфейс](/settings/services/connection-to-provider/README.md#туннельные-интерфейсы), указав в поле **Адрес удаленного интерфейса** IP-адрес WCCP-сервера.
