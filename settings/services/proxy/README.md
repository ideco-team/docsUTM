...
Пользователь запрашивает доступ к веб-странице:
* Запрос проходит через межсетевой экран, на котором настроен WCCP, и перенаправляется коммутатором на Ideco UTM;
* Ideco UTM является сервер кэширования и обрабатывает запрос:
  * Если запрос заблокирован, то информация о блокировке отправляется обратно пользователю.
  * Если запрос не заблокирован, то Ideco UTM проверяет включено ли кэширование трафика на диск (опция включается на вкладке **Основное**): 
    * Кэширование включено: Ideco UTM ищет копию запрашиваемой страницы в кэше:
      * Страница есть -> отправляется пользователю; 
      * Страницы нет -> выполняется действие, аналогичное последнему пункту.
    * Кэширование выключено: Ideco UTM направляет запрос на внешний сервер и ответ возвращается обратно по тому же пути, по которому уходил на внешний сервер.
...
Если на WCCP сервере задан пароль, то сохраните его в соответствующем поле.
