...
Master-зоны позволят использовать NGFW как сервер имен внутри сетевой инфраструктуры для обращения к IP-адресам хостов в сети по доменным именам. При настройке Master-зон базовые DNS-записи (NS, SOA, MX и статические A-записи) необходимо создавать вручную. Система не поддерживает автоматическую генерацию этих записей на основе DHCP или других внешних источников.

Однако для аутентифицированных пользователей и устройств NGFW поддерживает динамическое создание A- и PTR-записей на основе:
* Логина пользователя.
* Common Name (CN) из сертификата Device VPN.
* Названия устройства в ZTNA.
...
* **SOA** - описывает основные/начальные настройки зоны, или _определяет зону ответственности данного сервера_. Для каждой зоны должна существовать только одна запись SOA. В записи SOA указывается primary NS для домена и e-mail контактного лица и далее в скобках: \
...
* **SRV** - DNS-запись, определяющая серверы, отвечающие за работу конкретных сетевых служб в домене (например, Jabber и Active Directory);
* **NS** -  DNS-запись, указывающая авторитетные серверы, отвечающие за зону домена. Для NS-записей должны существовать соответствующие A/AAAA-записи.
* **PTR** - Обратная DNS-запись (reverse DNS, rDNS, PTR-запись), которая отображает IP-адрес в доменное имя.
* **MX** - DNS-запись, определяющая почтовые серверы, принимающие электронную почту для домена. Обычно имя домена почтового шлюза выглядит так: _mx.example.com_. Для MX хостов должны быть соответствующие A-записи;
...
Пример записей приведен на скриншоте ниже:
