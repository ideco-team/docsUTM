...
Master-зоны позволят использовать Ideco NGFW как сервер имен и переводить IP-адреса в доменные имена внутри сети.
...
* **SOA** - описывает начальные параметры зоны и определяет зону ответственности сервера. Для каждой зоны допустима только одна запись SOA. Включает primary NS-сервер домена и E-mail администратора (с заменой `@` на `.`, например: `admin.example.com`). Включает: \
  1\. **Serial** - серийный номер зоны. Должен увеличиваться при любых изменениях, чтобы вторичные серверы обновили данные. Рекомендуемый формат: `ГГГГММДДnn` (год, месяц, день + двузначный номер изменения, начиная с `00`). Например, первое изменение 15 мая 2025 года будет `2025051500`, следующее в тот же день — `2025051501`; \
  2\. **Refresh** - как часто вторичные серверы должны проверять первичный на наличие обновлений; \
  3\. **Retry** - интервал между повторными попытками при неудачном обновлении; \
  4\. **Expiry** - срок, после которого вторичный сервер прекратит использовать данные зоны, если не сможет обновиться; \
  5\. **TTL** - минимальное время кеширования для вторичных серверов.
