...
При настройке сторонних устройств необходимо явно указать криптоалгоритмы, используемые для подключения. Ideco UTM поддерживает самые современные и одновременно достаточно безопасные алгоритмы, не нагружающие сервер и устройства. При этом, не поддерживаются устаревшие и считающиеся небезопасными алгоритмы. При конфигурировании сторонних устройств, как правило, можно указать несколько поддерживаемых алгоритмов одновременно. В случае подключения к Ideco UTM лучше указать единственный алгоритм каждого вида. Список алгоритмов указан ниже в порядке убывания по приоритета для выбора.
...
* **Phase 1 \(IKE\):** 
  ...
  * integrity \(hash, целостность\):
    * для AES-GCM - не требуется, поскольку проверка целостности встроена в AEAD-алгоритмы;
  ...
  * prf \(функция генерации случайных значений\):
    ...
    * для AES-GCM может потребоваться указать явно. В этом случае по приоритетам: AESXCBC, SHA512, SHA384, SHA256
...
* **Phase 2 \(ESP\):**
  ...
  * integrity \(целостность\): 
    ...
    * для не GCM алгоритмов по приоритетам: SHA512, SHA384, SHA256.
