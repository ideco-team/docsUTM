...
* **UDP** - используется Ideco Client, с применением шифрования;
* **TLS (Transport Layer Security)** - используется Ideco Client, при подключении через TCP;
...
{% hint style="info" %}
При выборе **Любой** отмечаются все протоколы. Такое соединение может использовать любой из перечисленных протоколов.
{% endhint %}
