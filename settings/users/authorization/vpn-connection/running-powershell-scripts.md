...
{% hint style="warning" %}
Для подключения по VPN к Ideco UTM с белым IP адресом достаточно действий, указанных ниже. Если Ideco UTM выходит в интернет через маршрутизатор, воспользуйтесь пунктом [Подключение по VPN к Ideco UTM с доступом в интернет через маршрутизатор](#podklyuchenie-po-vpn-k-ideco-utm-s-dostupom-v-internet-cherez-marshrutizator).
{% endhint %}
...
2. Загрузите скрипт на компьютер, воспользовавшись 1 пунктом инструкции [Как запустить Powershell скрипт](#kak-zapustit-powershell-skript).
...
4. После выполнения действий следуйте инструкции [Как запустить Powershell скрипт](#kak-zapustit-powershell-skript) с 4 пункта.
