---
description: >-
  Как настроить аутентификацию пользователей в Ideco NGFW, используя контроллер домена на базе Microsoft Active Directory/Samba DC.
---
...
Для включения **SSO-аутентификации** перейдите на вкладку **Пользователи -> Авторизация -> Основное** и заполните поля: 
...
* **Авторизации через журнал безопасности Active Directory** - включите настройку, чтобы записывать события, связанные с проверкой прав доступа пользователей к ресурсам сети. Рекомендуем использовать совместно с SSO-аутентефикацией;
* **Резавторизация пользователей** - установите тайм-аут [разавторизации пользователей](settings\users\active-directory\auto-de-authorization-script.md). Значение по умолчанию - 15 минут. Диапазон доступных значений - от 10 минут до 1 дня;
* **Сохранить** - нажмите для сохранения.
...
## Настройка авторизации через журнал безопасности
...
![](/.gitbook/assets/active-directory2.png)
  ...
  * Откройте **Свойства** компьютера Ideco NGFW, введенного в домен (на скриншоте - idecongfw). Перейдите на вкладку **Член групп** и нажмите **Добавить**. В появившемся окне нажмите на **Дополнительно** и добавьте **Читатели журнала событий (Event Log Readers)** через **Поиск**:
