...
Netflow - проприетарный открытый протокол, разработанный Cisco для мониторинга статистики трафика в сети. Ideco NGFW выступает в качестве сенсора-экспортера, который собирает статистику при прохождении через него трафика. На вкладке можно настроить экспорт этой статистики на коллектор во внешней или локальной сети по протоколу Netflow. 

В Ideco NGFW доступны три версии протокола Netflow: Netflow 5, Netflow 9 и Netflow 10 (IPFIX).
...
* **Интервал отправки для активного потока, секунды** - введите временной интервал, через который NGFW будет отправлять отчеты на коллектор. По умолчанию - 300 секунд;
...
* **Интервал отправки шаблона, пакеты** - количество пакетов, через которое на коллектор будет послан шаблон, по умолчанию - 20. Доступно только при выборе Netflow 9 или Netflow 10 (IPFIX) в поле **Версия протокола**;
* **Интервал отправки шаблона, секунды** - количество секунд, через которое на коллектор будет послан шаблон, по умолчанию - 1800. Доступно только при выборе Netflow 9 или Netflow 10 (IPFIX) в поле **Версия протокола**.
...
При выборе в поле **Версия протокола** Netflow 9 или Netflow 10 (IPFIX) поля **Интервал отправки шаблона** фактически отображают допустимые потери данных, отправленных на коллектор, в случае потери связи между коллектором и NGFW (например, при перезагрузке одного из устройств): пока коллектор не получит шаблон передаваемых данных, отчеты будут игнорироваться.
{% endhint %}

## Стркутура передаваемых на коллектор данных
