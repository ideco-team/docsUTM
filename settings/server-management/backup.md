...
При выполнении мгновенного восстановления бекап применятся без перезагрузки. При этом не сохраняются:
...
* Автоматические бекапы отключены на резервной ноде. Все бекапы с активной ноды передаются на резервную в момент обмена данными (происходит раз в три минуты) и заменяют те бекапы, которые там присутствовали ранее;
* На активной ноде доступно **Мгновенное восстановление из бекапа**. Восстанавливаются все настройки, кроме использованного трафика по квотам, изменений в списке пользователей и отчетах. Данные на второй ноде также придут в соответствие с восстановленными настройкам после синхронизации нод;
