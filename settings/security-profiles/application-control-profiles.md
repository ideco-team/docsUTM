...
Профили безопасности применяются при создании и включении правил в разделе \
**Правила трафика -> Файрвол**. Чтобы трафик фильтровался модулем **Контроль приложений**, необходимо для всех локальных интерфейсов создать правило FORWARD, содержащее нужный профиль.

**Файрвол** Ideco NGFW VPP анализирует трафик, чтобы найти подходящее правило и применить его. Если в таблице несколько правил с одними и теми же условиями, применяется правило, стоящее выше по списку. Запрещающие правила **Файрвола** сразу блокируют соответствующий трафик и не проходит дополнительную проверку в модуле **Контроль приложений**.

{% hint style="warning" %}
Чтобы через модуль **Контроль приложений** проходил трафик, для которого нет разрешающего правила в таблицах **Файрвола**, рекомендуем создать и включить правило с источником и назначением **Любой**, разместив его в конец таблицы. В этом случае трафик, который не был найден в правилах **Файрвола**, пройдет проверку **Контролем приложений**. Трафик, не учтенный правилами **Файрвола**, но запрещенный профилями **Контроля приложений**, будет заблокирован.
{% endhint %}

## Создание профиля Контроля приложений

Для создания профиля **Контроля приложений** выполните действия:

1\. В разделе **Пользователи -> Учетные записи** добавьте пользователей, для которых будет настраиваться профиль. При необходимости объедините их в группы/подгруппы (например, в соответствии с организационной структурой компании).

{% hint style="info" %}
Рекомендуем настраивать профили **Контроля приложений** отдельно для каждой группы/подгруппы.
{% endhint %}

2\. Перейдите в раздел **Профили безопасности -> Контроль приложений** и нажмите **Добавить**.

3\. Заполните поля **Название** и **Комментарий** (необязательно) и добавьте профиль:

![](/.gitbook/assets/security-profiles1.png)

4\. В столбце **Управление** нажмите на ![](/.gitbook/assets/icon-edit.png), затем **Доступ к приложениям**.

5\. Выберите приложения, к которым будет применяться выбранное действие (**Разрешить** или **Запретить**):
...
6\. Нажмите **Применить**, чтобы определить действие для выбранных приложений.

7\. Нажмите **Сохранить** для применения настроек профиля.
...
3\. Включите опцию **Контроль приложений** в разделе **Профили безопасности** и из раскрывающегося списка выберите созданный профиль.
...
![](/.gitbook/assets/security-profiles5.png)

2\. Нажмите в столбце **Управление** на ![](/.gitbook/assets/icon-edit.png), затем **Доступ к приложениям**. 

3\. Выберите группы приложений, которые необходимо запретить (перечислены в описании примера):
...
![](/.gitbook/assets/security-profiles7.png)
...
![](/.gitbook/assets/security-profiles8.png)
...
![](/.gitbook/assets/security-profiles9.png)
...
<details>
<summary>Перечень доступных групп приложений и протоколов</summary>
