---
description: >-
  В статье описывается настройка VPN-подключения по протоколам L2TP/IPsec и IKEv2/IPsec.
---

# Инструкция по созданию подключения в Astra Linux

Перед настройкой VPN-подключения перейдите в **Пользователи -> VPN-подключения -> Доступ по VPN** и создайте разрешающее VPN-подключение правило.
...
1\. Перейдите в **Пользователи -> VPN-подключения -> Основное**.
...
* **Имя пользователя** - логин учетной записи пользователя NGFW;
* **Пароль** - пароль учетной записи. В правой части поля необходимо выбрать вариант хранения для пароля от VPN-соединения.
...
3\. Скачайте корневой сертификат Ideco NGFW в **Сервисы -> Сертификаты -> Загруженные сертификаты** в веб-интерфейсе NGFW или в личном кабинете пользователя по кнопке **Скачать корневой сертификат**.
...
* **Username** - логин учетной записи пользователя NGFW;
* **Password** - пароль учетной записи. В правой части поля необходимо выбрать вариант хранения для пароля от VPN-соединения.
