...
Рекомендуемый протокол подключения - IKEv2/IPsec.
...
Загружать сертфиикат **ISRG_ROOT_X1** сертификат не требуется, так как он уже есть в Fedora 40 и находится в каталоге `/etc/ssl/certs`.
...
* **User name** - имя пользователя на Ideco NGFW;
...
4\. В разделе **Идентификация** заполните следующие поля:
...
5\. В разделе **Идентификация** заполните следующие поля:
