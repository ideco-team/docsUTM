...
Инструкция актуальна для Fedora 41. Рекомендуемый протокол подключения - IKEv2/IPsec.
...
Загружать сертфикат **ISRG_ROOT_X1** не требуется, так как он уже есть в Fedora 41 и находится в каталоге `/etc/ssl/certs`.
