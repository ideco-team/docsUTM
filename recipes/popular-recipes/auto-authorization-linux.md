...
4\. Cоздайте в папке `ideco-utm` файл `auth.conf`. Откройте его, введите логин и пароль пользователя по образцу:
...
5\. Cкачайте скрипт и настройте автозагрузку в соответствии с описанным ниже алгоритмом:
...
2\. Скопируйте в файл список, введите имя пользователя Linux и полный путь до скрипта с указанием его названия:
...
Проверить работу настройки можно с помощью интерфейса Ideco NGFW. Зайдите в раздел **Мониторинг -> Авторизованные пользователи** и прервите сессию пользователя, для которого настраивалась автоматическая аутентификация. Чтобы прервать сессию пользователя, нажмите на соответствующую пиктограмму и подтвердите свой выбор. 

![](/.gitbook/assets/linuxauth1.png)

При обновлении страницы браузера с устройства пользователя страница авторизации появиться не должна.