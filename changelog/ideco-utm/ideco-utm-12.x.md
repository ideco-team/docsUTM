...
* Добавлена возможность создать правило авторизации только по MAC-адресу;
* Добавлена фильтрация баннерной рекламы на уровне DNS;
* Добавлена динамическая маршрутизация на базе OSPF;
* Добавлена возможность возврата на прошлую версию после обновления;
* Добавлено резервирование IP к MAC в разделе DHCP;
* Добавлена возможность запускать веб-интерфейс для Антиспама;
...
* Правила Предотвращения вторжений, Контроля приложений и Ограничение скорости обрабатывают поступающий по VPN трафик из внешней сети;
...
* Правила Предотвращения вторжений и Ограничение скорости перестали обрабатывать трафик между локальными сетями и сетями филиалов;
* Переработан раздел с сертификатами;
* Переработан и улучшен раздел авторизации пользователей по IP-адресу/MAC-адресу;
...
* Обнаружение устройств теперь создает авторизацию по MAC для локальных адресов в одном ethernet-сегменте (ранее создавалась авторизация по IP);
