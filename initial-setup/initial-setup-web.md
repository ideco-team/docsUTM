...
{% hint style="danger" %}
Для [получения доступа в интернет](/installation/get-internet.md) через Ideco NGFW необходимо создать учетную запись (администратора/пользователя) и настроить авторизацию. В противном случае доступ в интернет для устройства с установленным Ideco NGFW VPP будет заблокирован.
{% endhint %}
...
Далее потребуется зарегистрировать сервер в личном кабинете. Подробнее смотрите в статье [Регистрация сервера](/initial-setup/server-registration.md).