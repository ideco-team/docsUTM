...
* `template_tx_counter` - при значениях `9` или `10` в поле `version` - количество пакетов, через которое на коллектор будет послан шаблон. Минимум `10`, максимум `6000`. При значении `5` в поле `version` должно быть `null`;
* `template_tx_interval` - при значениях `9` или `10` в поле `version` - количество секунд, через которое на коллектор будет послан шаблон. Минимум `60`, максимум `86400`. При значении `5` в поле `version` должно быть `null`.
