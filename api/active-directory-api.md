...
## Управления аккаунтами администраторов

<details>
<summary>Получение списка администраторов</summary>
...
* `id` - идентификатор записи. Для локальных администраторов идентифицирует еще и самого администратора;
* `enabled` - аккаунт включен/выключен (можно/нельзя под ним зайти в систему);
* `name` - для локальных администраторов имя администратора. Для AD/ALD администраторов пустое значение;
* `login` - логин, может быть 'null' у администраторов из AD/ALD;
* `role` - идентификатор уровня доступа аккаунта;
* `group_alias` - алиас группы безопасности в которую входят AD/ALD администраторы. Для локальных администраторов null;
* `auth_type` - тип способа авторизации администратора: "ad","ald","local";
* `domain` - домен в котором состоят администраторы из группы безопасности. Для локальных администраторов пустое поле;
* `comment` - момментарий;
...
* `login` - логин не должен содержать `.` (одну точку) или `..` (две точки), а также символов, соответствующих регулярному выражению [\\:/~$!\s]. Длинна поля от 1 до 42 символов включительно;
* `password` - пароль, ненулевое текстовое поле, длина от 10 до 42 ASCII символов;
* `role` - идентификатор уровня доступа аккаунта;
...
<details>
<summary>Удаление администратора</summary>

**Последнего локального администратора удалять нельзя!**

```
DELETE /web/admins/$ID
```
...
<details>
<summary>Авторизация администратора в Web-интерфейсе</summary>
...
**Ответ на успешный запрос:** Статус 200 и пустое тело. 401 с текстовым (не JSON) описанием ошибки если авторизация не успешна. 542 в случае ошибок (невозможности) проверки логина/капчи/пароля и т.д.
